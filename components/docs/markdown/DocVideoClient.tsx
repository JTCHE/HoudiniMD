...
export interface DocVideoClientProps {
  src: string;
  title?: string;
  /** Probed `width / height` string, or null when probing found nothing. */
  probedRatio: string | null;
}

/**
 * The real aspect ratio is known up front from the server-side header probe
 * (see lib/videos/probe.ts), so the box locks in at the right size on first
    ...
    <div
      ...
      onLoadedMetadataCapture={(e) => {
        ...
        if (video?.videoWidth && video?.videoHeight) {
          setFallbackRatio(`${video.videoWidth} / ${video.videoHeight}`);
        }
      }}
    >
      <MediaPlayer
